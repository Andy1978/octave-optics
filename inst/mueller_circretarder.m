...
## @deftypefnx {Function File} {@var{M} =} mueller_circretarder(@var{p})
## @deftypefnx {Function File} {@var{M} =} mueller_circretarder(..., @var{mode})
...
## @end enumerate
##
## @seealso{mueller_waveplate, mueller_linretarder}
## @end deftypefn
...
%! A = mueller_circretarder();
%! R = A*A-A;
%! assert(norm(R,inf), 0, 1e-9);
%!
%!test
%! % test serial application of circular retarder elements
%! phase = rand(1, 1);
%! A1 = mueller_circretarder(phase);
%! A2 = mueller_circretarder(phase*2);
%! R = A1*A1-A2;
%! assert(norm(R,inf), 0, 1e-9);
%!
%!test
...
%! phase1 = rand(1, 1);
%! phase2 = rand(1, 1);
%! A1 = mueller_circretarder(phase1);
%! A2 = mueller_circretarder(phase2);
%! A12 = mueller_circretarder(phase1+phase2);
%! R = A1*A2-A12;
%! assert(norm(R,inf), 0, 1e-9);
%!
%!test
...
%! phase = rand(1, 1);
%! A1 = mueller_circretarder(phase);
%! A2 = mueller_circretarder(phase*180/pi(), 'deg');
%! A3 = mueller_circretarder(phase/(2*pi()), 'wav');
%! R1 = A1-A2;
%! R2 = A1-A3;
